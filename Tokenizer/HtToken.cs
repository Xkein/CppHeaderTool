...
namespace CppHeaderTool.Tokenizer
{
    ...
    /// </summary>
    public static class HtTokenTypeExtensions
    {
        ...
        public static bool IsEndType(this HtTokenType tokenType)
        {
            switch (tokenType)
            {
                case HtTokenType.EndOfFile:
                case HtTokenType.EndOfDefault:
                case HtTokenType.EndOfType:
                case HtTokenType.EndOfDeclaration:
                    return true;
                default:
                    return false;
            }
    ...
    public struct HtToken
    {
        ...
        /// Type of the token
        /// </summary>
        public HtTokenType TokenType { get; set; }
        ...
        public HtToken(HtTokenType tokenType)
        {
            TokenType = tokenType;
            UngetPos = 0;
            UngetLine = 0;
        ...
        public HtToken(HtTokenType tokenType, int ungetPos, int ungetLine, int inputStartPos, int inputLine, StringView value)
        {
            TokenType = tokenType;
            UngetPos = ungetPos;
            UngetLine = ungetLine;
        ...
        public static implicit operator bool(HtToken token)
        {
        ...
        {
            return TokenType == HtTokenType.Identifier;
        }
        ...
        {
            return TokenType == HtTokenType.Symbol;
        }
        ...
        {
            return TokenType == HtTokenType.DecimalConst || TokenType == HtTokenType.HexConst;
        }
        ...
        {
            return TokenType == HtTokenType.FloatConst;
        }
        ...
        public bool GetConstInt(out int value)
        {
            switch (TokenType)
            {
                case HtTokenType.DecimalConst:
                    value = (int)GetDecimalValue();
                    return true;
                case HtTokenType.HexConst:
                    value = (int)GetHexValue();
                    return true;
                case HtTokenType.FloatConst:
                    {
        ...
        {
            return TokenType == HtTokenType.StringConst || TokenType == HtTokenType.CharConst;
        }
        ...
        /// <exception cref="HtException">Thrown if the token type is not a string or character constant</exception>
        public StringView GetUnescapedString()
        {
            switch (TokenType)
            {
                case HtTokenType.StringConst:
            ...
            throw new Exception("Call to GetUnescapedString on token that isn't a string or char constant");
        }
        ...
        public StringView GetTokenString(bool respectQuotes = false)
        {
            ...
            switch (TokenType)
            {
                ...
                default:
                    throw new Exception("Call to GetTokenString on a token that isn't a string or char constant");
            }
        ...
        public static string Join(IEnumerable<HtToken> tokens)
        {
            StringBuilder builder = new();
            foreach (HtToken token in tokens)
            {
        ...
        /// <param name="tokens">Tokens to join</param>
        /// <returns>Joined strings</returns>
        public static string Join(char separator, IEnumerable<HtToken> tokens)
        {
            StringBuilder builder = new();
        ...
        /// <param name="tokens">Tokens to join</param>
        /// <returns>Joined strings</returns>
        public static string Join(string separator, IEnumerable<HtToken> tokens)
        {
            StringBuilder builder = new();
            bool includeSeperator = false;
            foreach (HtToken token in tokens)
        ...
        private float GetFloatValue()
        {
            ...
            {
                if (HtFCString.IsFloatMarker(Value.Span[^1]))
                {
        ...
        long GetDecimalValue()
        {
            ...
            {
                char c = span[^1];
                if (HtFCString.IsLongMarker(c))
                {
                    span = span[0..^1];
                }
                else if (HtFCString.IsUnsignedMarker(c))
                {
                    isUnsigned = true;
                    span = span[0..^1];
                }
